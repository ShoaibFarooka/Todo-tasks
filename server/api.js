...
exports.setApp = function (app, client) {
  ...
  app.patch('/api/reset-password/:UserID', async (req, res) => {
    try {
      ...
      if (user) {
        await db.collection('userotp').deleteOne({ userID: new ObjectId(req.params.UserID) });
    ...
    try {
      ...
      if (user) {
        ...
        if (existingUserCode) {
          ...
          const updatedUserCode = await db.collection('userotp').findOneAndUpdate(
            { userID: user._id },
            { $set: { OTP: randomOTP } },
            { returnOriginal: false }
          );
        ...
        const mailOptions = {
          ...
          text: `Here is your 6 digits OTP "${randomOTP}" to verify email and link to reset password is ${origin + 'reset-password/' + user._id}`,
        };
        transporter.sendMail(mailOptions, (error, info) => {
          if (error) {
            console.log('Error sending email:', error);
