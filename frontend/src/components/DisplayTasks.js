...
function DisplayTasks({ updateTask }) {
  ...
  const handleEdit = async (task) => {
    if (task._id === editTaskID) {
      ...
      try {
        const response = await axios.put(bp.buildPath(`api/updatetask/${task._id}`), Data);
  ...
  const toggleSelect = async (e, id) => {
    ...
    try {
      const response = await axios.put(bp.buildPath(`api/updatetask/${id}`), Data);
      if (response.status === 200) {
  ...
  const handleDelete = async (task) => {
    try {
      const response = await axios.delete(bp.buildPath(`api/deletetask/${task._id}`));
      if (response.status === 200) {
        ...
        <table className="tasks-table">
          ...
          <tbody>
            {tasks.map((task, index) => (
              <tr key={index} className={task.TaskCompleted ? 'completed-task' : ''}>
                <td className="data">
                  {task._id === editTaskID ?
                ...
                <td className="data">
                  {task._id === editTaskID ?
                    <input type="text" name="TaskDescription" value={formData.TaskDescription} onChange={handleChange} />
                    : task.TaskDescription}
                </td>
                <td className="data">
                  {task._id === editTaskID ?
                    <input type="text" name="TaskDifficulty" value={formData.TaskDifficulty} onChange={handleChange} />
                    : task.TaskDifficulty}
                </td>
                <td className="data">
                  <input type="checkbox" name="TaskCompleted" checked={checkedTaskID === task._id ? formData.TaskCompleted : task.TaskCompleted} onChange={(e) => toggleSelect(e, task._id)} />
                </td>
                <td>
                  <button onClick={() => handleEdit(task)}>{task._id === editTaskID ? 'Update' : 'Edit'}</button>
                </td>
                <td>
