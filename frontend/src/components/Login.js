...
function Login() {
  ...
  const handleForgetPassword = async () => {
    ...
    try {
      const res = await axios.post(bp.buildPath('api/send-reset-link'), { Email: email });
      if (res.status === 200) {
      ...
      <div className="intermediary-container">
        {fogetPass ?
          <div className="forget-pass-div">
            ...
            <p style={{ color: 'red' }} id="loginResult" className="login-message">{message}</p>
          </div>
          :
          <form onSubmit={doLogin} className="login-form" id="loginDiv">
            ...
            <div className="forget-para-div">
              <p className="forget-para" onClick={(e) => setForgetPass(true)}>Forget Password?</p>
            </div>
